...
Price-based analysis begins from a different premise: rather than asking what an asset should be worth, it asks what the market itself is communicating through the behavior of prices and volumes. Its theoretical backdrop is the efficient market hypothesis — and the debate about how efficient markets actually are.

The EMH, as formalized by Fama, holds that prices reflect available information, but the claim has three distinct forms that are routinely treated as equivalent [@fama1970]. The weak form states that no trading rule based on historical price data can generate abnormal returns. The semi-strong form extends this to all publicly available information. The strong form extends it to all information, including private. The semi-strong form is broadly well-supported in liquid markets. The strong form has long been known to be false — insider trading produces abnormal returns. The weak form is more contested.
...
Both fundamental and price-based methods are ultimately attempting to answer the same question: has the market made an error, and if so, is that error likely to correct within a relevant horizon? The distinction between them is methodological — one starts from economic value, the other from market behavior — but both are subject to the same competitive pressure that makes persistent outperformance rare. Every genuine inefficiency identified by either method attracts capital, and that capital erodes the advantage. This is the mechanism behind signal decay, which the next section examines directly.
